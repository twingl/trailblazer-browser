(function() {
  ...
  window.ChromeAuth = {
    authenticate: function(resolve, reject) {
    ...
    invalidate: function(data, resolve, reject) {
      var body, url;
      body = {
  ...
  OAuth2Authenticator = SimpleAuth.Authenticators.Base.extend({
    ...
    authenticate: function(credentials) {
      return new Ember.RSVP.Promise(function(resolve, reject) {
        return ChromeAuth.authenticate(resolve, reject);
      });
    },
    invalidate: function(data) {
      return new Ember.RSVP.Promise(function(resolve, reject) {
        return ChromeAuth.invalidate(data, resolve, reject);
      });
    }
  });
...
(function() {
  Twingl.BrowserRoute = Ember.Route.extend(SimpleAuth.AuthenticatedRouteMixin, {
    model: function() {
      var id, url;
      id = this.controllerFor('application').get('assignment').id;
      url = "" + window.ENV['api_base'] + "/assignments/" + id + "/nodes";
      return Ember.$.getJSON(url);
    },
    setupController: function(controller, model) {
      var currentNodeId, node, _i, _len, _ref;
      controller = this.controllerFor('tree');
      if (model.nodes.length > 0) {
        ...
        if (currentNodeId && controller.get('historyMap').hasOwnProperty(currentNodeId)) {
        ...
        setTimeout(((function(_this) {
          return function() {
            return controller.get('navigation').send('historyShow');
          };
        })(this)), 40);
        return controller.get('webview').navigate(controller.currentNode().url, false);
      } else {
        setTimeout(((function(_this) {
          return function() {
            return controller.get('navigation').send('browserShow');
          };
        })(this)), 40);
        return controller.get('webview').navigate(window.ENV['default_page']);
      }
    },
    renderTemplate: function() {
