...
(function() {
  Twingl.TreeController = Ember.Controller.extend({
    ...
    d3data: {
      ...
      height: 1,
      width: 1,
      force: d3.layout.force().size([100, 100]).charge(-1600).chargeDistance(1600).linkDistance(200).linkStrength(1).friction(0.95).gravity(0)
    },
    ...
    actions: {
      ...
      pushItem: function(node) {
        ...
        Ember.$.post(url, {
          ...
          return function(response) {
            ...
            _this.set("currentNodeId", node.id);
      ...
      drawTree: function() {
        ...
        if (this.get("historyStack").length > 0) {
          this.update();
          return svgPanZoom("#tb-history-tree-viz>svg", {
    ...
    update: function() {
      var force, history, id, idMap, index, label, link, links, map, node, nodes, poly, svg;
      ...
      link = svg.selectAll(".link").data(links).enter().append("line").attr("class", "link").style("stroke-width", 4);
      node = svg.selectAll('g.node').data(nodes, function(d) {
        return d.id;
      }).enter().append("g").attr("class", "node");
      poly = node.append("polygon").attr("stroke-width", "20").attr("stroke-linejoin", "round").attr("stroke", this.get("d3data").nodes.color).style("fill", this.get("d3data").nodes.color);
